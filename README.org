...
Likewise, enable the [[https://elpa.nongnu.org/nongnu-devel/][NonGNU-devel ELPA Packages]].
...
4. Write a macro ~timestamp~ such that ~timestamp~ expands into a
   number literal counting the number of uses of ~timestamp~.

5. Rewrite ~fluid-let~ as a recursive macro that does not use
   ~generate-temporaries~.

6. Write a procedure ~symbolic-identifier=?~ so that
   ~(symbolic-identifier=? id1 id2)~ returns ~#t~ if and only if the
   two identifiers ~id1~ and ~id2~ have the same symbolic name.

7. Extend the ~loop~ macro so that evaluating ~(continue)~ skips the
   rest of the current loop iteration and the loop continues with the
   next iteration.

8. Modify the ~for~ macro into a functional form supporting
   user-definable accumulators whose final result are returned by the
   ~for~ expression.

9. Write a pattern matching Scheme macro.

10. Make the parser generator more user friendly.  It should provide
...
11. Write a version of the lex scanner generator as a Scheme macro.
